<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateSoftCostITBTrackersTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('soft_cost_i_t_b_trackers', function (Blueprint $table) {
            $table->id();

             $table->unsignedBigInteger('project_id');
            $table->foreign('project_id')->references('id')->on('projects')->onDelete('cascade');

            $table->unsignedBigInteger('soft_cost_trade_id');
            $table->foreign('soft_cost_trade_id')->references('id')->on('soft_cost_trades')->onDelete('cascade');

            $table->unsignedBigInteger('soft_cost_vendor_id');
            $table->foreign('soft_cost_vendor_id')->references('id')->on('soft_cost_vendors')->onDelete('cascade');
                
            $table->tinyInteger('mail_sent')->default(0);
            $table->tinyInteger('bid_recieved')->default(0);
            $table->tinyInteger('contract_sign')->default(0);

            
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('soft_cost_i_t_b_trackers');
    }
}
