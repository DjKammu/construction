...
<DIV id="page_1">
      <div style="text-align:center; width: 100%; margin-bottom: 10px;margin-left: 75px;">
        <P style="text-align:center;" class="p0 ft0"> {{ @$project->name }} Awarded Contracts</P>
    </div>
      ...
      $catBudgetDiff = 0;
        ...
        @foreach($catTrades as $k => $trd)
            ...
            <TR>
                  ...
                  $bidTotal =  (float) @$bid->material + (float) @$bid->labour_cost + (float) @$bid->subcontractor_price;   
                     foreach(@$bid->changeOrders as $k => $order){
                      ...
                      // $catbudgetDiff = (float) @$bid->trade_budget - @$bidTotal +  @$catbudgetDiff;
                       ...
                       $subcontractorName = (@$bid->subcontractor->name) ? $bid->subcontractor->name : '';
              ...
              <TD class="tr5 td6 {{ $bb }}" ><P class="p5 ft9">{{ @$subcontractorName }}</P> </TD>
              <TD class="tr5 td6 {{ $bb }}" ><P class="p5 ft9">${{ \App\Models\Payment::format($catGrandTotal) }} </P></TD>
              <TD class="tr5 td6 {{ $bb }}" ><P class="p5 ft9">${{ \App\Models\Payment::format($catPaidTotal) }}</P></TD>
              <TD class="tr5 td6 {{ $bb }}" ><P class="p5 ft9">${{ \App\Models\Payment::format($catDueTotal) }} </P></TD>
              <TD class="tr5 td6 {{ $bb }}" ><P class="p5 ft9">{{ ($catGrandTotal) ? sprintf('%0.2f', (@$catPaidTotal) / (@$catGrandTotal) * 100) : 0 }} %  </P></TD>
            </TR>
