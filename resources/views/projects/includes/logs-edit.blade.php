...
@include('includes.back', ['url' => route("projects.show", ['project' => @$log->project->id]),
'to' => 'to Project'])
              ...
              <div class="row mb-2">
               ...
               <div class="row">
                        <div class="col-md-12">
                            <div class="card-body">
                                <form   method="post" 
                              action="{{ route('projects.logs.update',['id' => request()->id]) }}"
                                              ...
                                              <div class="form-group">
                                                  ...
                                                  <select class="form-control" name="vendor_id"> 
                                                    ...
                                                    @foreach(@$vendors as $vendor)
                                                     <option value="{{ $vendor->id }}" {{ (@$log->vendor_id == $vendor->id) ? 'selected' : ''}}> {{ $vendor->name}}
                                                     </option>
                                                    @endforeach
                                                  </select>
                                              </div>
                                              ...
                                              <div class="form-group">
                                                  ...
                                                  <select class="form-control" name="subcontractor_id"> 
                                                    ...
                                                    @foreach(@$subcontractors as $subcontractor)
                                                     <option value="{{ $subcontractor->id }}" {{ (@$log->subcontractor_id == $subcontractor->id) ? 'selected' : ''}} >{{ $subcontractor->name}}
                                                     </option>
                                                    @endforeach
                                                  </select>
                                              </div>
                                    ...
                                    <tr class="text-center col-lg-2 col-sm-3 odd" style="display: flex; flex-wrap: wrap;" role="row">
                                       <td>
                                            <span class="cross"> 
                                             <form 
                                                method="post" 
                                                action="{{route('projects.logs.file.destroy', ['id' => request()->id])}}?path={{$file}}"> 
                                                 @csrf
                                                {{ method_field('DELETE') }}
