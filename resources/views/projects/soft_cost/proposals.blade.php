 <div class="tab-pane" id="proposals" role="tabpanel" aria-expanded="true">
    ...
    <div class="row mb-2">
        ...
        <div class="col-6">
            <h4 class="mt-0 text-left">{{ @$project->name }} - Soft Cost Proposals List </h4>
        </div>
        <div class="col-6 text-right">

            <button type="button" class="btn btn-danger mt-0"  onclick="return window.location.href='{{ route("projects.soft-cost.proposals",['project' => request()->project, 
      ...
      <div class="card card-user card-table-item" style="width: 100%; height: 100%;">
         ...
         <div class="card-body">
            ...
            <div class="row">
               ...
               <div class="col-6">
                <form 
                class="file_form" action="{{route('projects.soft-cost.proposals.upload',['project' => request()->project, 'id' => $proposal->id]).'#proposals'}}" 
                method="post" 
         ...
         <div class="card-footer text-center">
            ...
            ['project' => request()->project ,'id' => $proposal->id, 'status' => $proposal->awarded ])  }}'" type="button" class="btn btn-success"  {{ ($proposal->awarded == 0 && $awarded == 1 ) ? 'disabled' : '' }}>
            {{ ($proposal->awarded == 1) ? 'Retract' : 'Award'  }}
          </button>                
            <button onclick="return window.location.href='{{ route("projects.soft-cost.proposals.edit",
            ['project' => request()->project ,'id' => $proposal->id ])  }}'" type="button" class="btn btn-warning">Edit</button> 
            <form  style="display: initial;" 
              method="post" 
              action="{{route('projects.soft-cost.proposals.destroy',['project' => request()->project, 'id' => @$proposal->id]).'#proposals'}}"> 
               @csrf
              {{ method_field('DELETE') }}
              <button 
                type="submit"
