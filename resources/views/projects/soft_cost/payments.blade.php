<div class="tab-pane" id="payments" role="tabpanel" aria-expanded="true">
   <div class="row mb-2">
         <div class="col-6">
            <h4 class="mt-0 text-left">{{ @$project->name }} - Soft Cost Payments List </h4>
        </div>
        <div class="col-6 text-right">
            <button type="button" class="btn btn-danger mt-0"  onclick="return window.location.href='{{ route("projects.soft-cost.payments",['project' => request()->project ])  }}'">Add Payment
            </button>
        </div>
 ...
 <div class="table-responsive table-payments">
       ...
       <table class="table table-hover text-center payments-table">
            <thead>
            ...
            <td>
              ...
              @endif
               ...
               <td>{{ @\App\Models\Payment::$statusArr[$payment->status] }}</td>
                  <td>        
                    <button onclick="return window.location.href='soft-cost/payments/{{$payment->id}}'" rel="tooltip" class="btn btn-neutral bg-transparent btn-icon" data-original-title="Edit Project Type" title="Edit Project Type">            <i class="fa fa-edit text-success"></i>        
                    </button> 
                  </td>
              <td>
                 <form 
                  method="post" 
                  action="{{route('projects.soft-cost.payments.destroy',['project' => request()->project, 'id' => $payment->id]).'#payments'}}"> 
                   @csrf
                  {{ method_field('DELETE') }}
