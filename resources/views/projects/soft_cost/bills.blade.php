<div class="tab-pane" id="bills" role="tabpanel" aria-expanded="true">
   <div class="row mb-2">
         <div class="col-6">
            <h4 class="mt-0 text-left">{{ @$project->name }} - Soft Cost Bills List </h4>
        </div>
        <div class="col-6 text-right">
            <button type="button" class="btn btn-danger mt-0"  onclick="return window.location.href='{{ route("projects.soft-cost.bills",['project' => request()->project ])  }}'">Add Soft Cost Bill
            </button>
        </div>
 ...
 <div class="table-responsive table-payments">
       ...
       <table id="project-types-table" class="table table-hover text-center payments-table">
            ...
            <td>        
              <button onclick="return window.location.href='soft-cost/bills/{{$bill->id}}'" rel="tooltip" class="btn btn-neutral bg-transparent btn-icon" data-original-title="Edit Project Type" title="Edit Project Type">            <i class="fa fa-edit text-success"></i>        
              </button> 
            </td>
              <td>
                 <form 
                  method="post" 
                  action="{{route('projects.soft-cost.bills.destroy',['project' => request()->project,
                  'id' => $bill->id]).'#bills'}}"> 
                   @csrf
                  {{ method_field('DELETE') }}
