    ...
    <ul class="list"> 
         ...
         @foreach($catTrades as $trd)
            ...
            <li class="multi-line">
              ...
              @php
                  $bids = @$project->ffe_proposals()->trade($trd->id)->get();
                  $bidCount = @$bids->count();
              ...
              @foreach($bids as $bid)
                ...
                @endphp
                <span  style="width:{{$spanWidth}}%;" class="text-center bid-text {{ (@$bid->awarded) ? 'awarded-green' : '' }}"><a onclick="proposalPage('{{@$trd->id }}')"  href="javascript:void(0)">{{ $bid->vendor->name }} </a> <br><b> {{ ($bidTotal) ? '$'.\App\Models\Payment::format($bidTotal)
                  : "No Bid" }} </b></span>
              @endforeach
