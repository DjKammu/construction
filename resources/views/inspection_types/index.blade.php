    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card-body">
                <div class="row mb-2">
                    <div class="col-6">
                        <h4 class="mt-0 text-left">Inspection Type List</h4>
                    </div>
                    <div class="col-6 text-right">
                        <button type="button" class="btn btn-danger mt-0"  onclick="return window.location.href='inspection-types/create'">Add Inspection Type
                        </button>
                    </div>
                </div>
                <!-- Categories Table -->
                <div class="table-responsive">
                    <table id="project-types-table" class="table table-hover text-center">
                        ...
                        <tr class="text-danger">
                            ...
                            <th>Inspetion Type <span class="sorting-outer">
                        ...
                        <tbody>
                          @foreach($inspetionTypes as $type)
                         <tr>
                           <td> {{ $type->account_number }}</td>
                           <td>{{ $type->name }}</td>
                           <td>        
                            <button onclick="return window.location.href='inspection-types/{{$type->id}}'" rel="tooltip" class="btn btn-neutral bg-transparent btn-icon" data-original-title="Edit Inspection Type" title="Edit Inspection Type">            <i class="fa fa-edit text-success"></i>        </button> 
                          </td>
                          <td>
                             <form 
                              method="post" 
                              action="{{route('inspection-types.destroy',$type->id)}}"> 
                               @csrf
                              {{ method_field('DELETE') }}
                              ...
                              <button 
                                ...
                                class="btn btn-neutral bg-transparent btn-icon" data-original-title="Delete Inspection Type" title="Delete Inspection Type"><i class="fa fa-trash text-danger"></i> </button>
                            </form>
                           </td>
                         </tr> 
                         @endforeach
